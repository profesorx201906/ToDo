...
export default function TodoItemAdd() {
  ...
  useDocumentTitle(t('todoItem.add.headline'));
  ...
  const createTodoItem = async (data: TodoItemDTO) => {
    window.scrollTo(0, 0);
    try {
      await axios.post('/api/todoItems', data);
      navigate('/todoItems', {
            state: {
              msgSuccess: t('todoItem.create.success')
            }
          });
    } catch (error: any) {
  ...
  return (<>
    <div className="d-flex flex-wrap mb-4">
      <h1 className="flex-grow-1">{t('todoItem.add.headline')}</h1>
      <div>
        <Link to="/todoItems" className="btn btn-secondary">{t('todoItem.add.back')}</Link>
      </div>
    </div>
    <form onSubmit={useFormResult.handleSubmit(createTodoItem)} noValidate>
      ...
      <input type="submit" value={t('todoItem.add.headline')} className="btn btn-primary mt-4" />
    </form>
  </>);
}
